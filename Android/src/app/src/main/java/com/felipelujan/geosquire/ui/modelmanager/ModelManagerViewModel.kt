...
import com.felipelujan.geosquire.AppLifecycleProvider
import com.felipelujan.geosquire.BuildConfig
import com.felipelujan.geosquire.common.getJsonResponse
import com.felipelujan.geosquire.data.AGWorkInfo
import com.felipelujan.geosquire.data.Accelerator
import com.felipelujan.geosquire.data.Config
import com.felipelujan.geosquire.data.DataStoreRepository
import com.felipelujan.geosquire.data.DownloadRepository
import com.felipelujan.geosquire.data.EMPTY_MODEL
import com.felipelujan.geosquire.data.IMPORTS_DIR
import com.felipelujan.geosquire.data.Model
import com.felipelujan.geosquire.data.ModelAllowlist
import com.felipelujan.geosquire.data.ModelDownloadStatus
import com.felipelujan.geosquire.data.ModelDownloadStatusType
import com.felipelujan.geosquire.data.TASKS
...
import com.felipelujan.geosquire.data.createLlmChatConfigs
import com.felipelujan.geosquire.data.getModelByName
import com.felipelujan.geosquire.data.processTasks
import com.felipelujan.geosquire.proto.AccessTokenData
import com.felipelujan.geosquire.proto.ImportedModel
import com.felipelujan.geosquire.proto.Language
import com.felipelujan.geosquire.proto.Theme
import com.felipelujan.geosquire.ui.common.AuthConfig
import com.felipelujan.geosquire.ui.llmchat.LlmChatModelHelper
...
constructor(
  ...
  fun initializeModel(context: Context, task: Task, model: Model, force: Boolean = false) {
    viewModelScope.launch(Dispatchers.Default) {
      ...
      when (task.type) {
        ...
        TaskType.LLM_ASK_HAND_SAMPLE,
        TaskType.LLM_ANALYZE_OUTCROP,
  ...
  fun readLanguageOverride(): Language {
    return dataStoreRepository.readLanguage()
  }

  fun saveLanguageOverride(language: Language) {
    dataStoreRepository.saveLanguage(language = language)
  }
  ...
  fun addImportedLlmModel(info: ImportedModel) {
    ...
    for (task in
      listOf(TASK_LLM_ASK_IMAGE, TASK_LLM_ASK_HAND_SAMPLE, TASK_LLM_ANALYZE_OUTCROP, TASK_LLM_ASK_AUDIO, TASK_LLM_PROMPT_LAB, TASK_LLM_CHAT)) {
      ...
      if (
        ...
        (task == TASK_LLM_ASK_HAND_SAMPLE && model.llmSupportImage) ||
        (task == TASK_LLM_ANALYZE_OUTCROP && model.llmSupportImage) ||
          (task == TASK_LLM_ASK_AUDIO && model.llmSupportAudio) ||
          (task != TASK_LLM_ASK_IMAGE && task != TASK_LLM_ASK_HAND_SAMPLE && task != TASK_LLM_ANALYZE_OUTCROP && task != TASK_LLM_ASK_AUDIO)
      ) {
  ...
  fun loadModelAllowlist() {
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        TASK_LLM_ASK_HAND_SAMPLE.models.clear()
        TASK_LLM_ANALYZE_OUTCROP.models.clear()
        ...
        for (allowedModel in modelAllowlist.models) {
          ...
          if (allowedModel.taskTypes.contains(TASK_LLM_ASK_IMAGE.type.id)) {
            ...
            TASK_LLM_ASK_HAND_SAMPLE.models.add(model)
            TASK_LLM_ANALYZE_OUTCROP.models.add(model)
          }
