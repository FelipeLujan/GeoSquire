...
class DefaultDataStoreRepository(private val dataStore: DataStore<Settings>) : DataStoreRepository {
  ...
  override fun saveLanguage(language: Language) {
    runBlocking {
      dataStore.updateData { settings -> settings.toBuilder().setLanguage(language).build() }
    }
  }

  override fun readLanguage(): Language {
    return runBlocking {
      val settings = dataStore.data.first()
      val curLanguage = settings.language
      // Use English as the default language.
      if (curLanguage == Language.LANGUAGE_UNSPECIFIED) Language.LANGUAGE_ENGLISH else curLanguage
    }
  }
}
